...
{
    stackinterpreter::instruction_tuple instruction_buffer = stackinterpreter::InstructionHandler::handle_instruction(ui->instructions_select->currentIndex(), ui->flagged_values->text());
    // Formatter and memory log goes here
    ui->stackostream->setText(QString::fromStdString(std::to_string((instruction_buffer.value))) + " ");
    qDebug() << "Val1: " << instruction_buffer.instruction << " -- Val2: " << instruction_buffer.value;
}
